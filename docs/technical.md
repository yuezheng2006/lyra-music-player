...
1. Arch Linux / Manjaro：通过 AUR 安装 `lyra-music-player-bin`

```bash
yay -S lyra-music-player-bin
```

2. Debian / Ubuntu / Linux Mint：下载 `.deb`
3. Fedora / RHEL / openSUSE：下载 `.rpm`
4. 其他发行版：下载 `tar.gz`，解压后直接运行 `lyra-music-player`
...
- 汽水音乐：支持关键词搜索与试听播放（LunaPC 官方 search + 分享页试听链）；分享短链仍可作为兜底。桌面端可走官方网页扫码，把 Douyin / 汽水会话 Cookie 带到 sidecar；`track_v2` 高音质与 `play_auth` 解密尚未接入，歌单/红心不同步。
- 酷狗音乐：关键词搜索、免费曲试听；桌面端可扫码登录，用用户自己的 `token` / `userid` 走 tracker 取播（失败回落 playInfo）。不自动签到或领取概念 VIP。LRC 歌词。
...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/yuezheng2006/lyra-music-player)
